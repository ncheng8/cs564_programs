...
namespace badgerdb { 
	...
	void BufMgr::allocBuf(FrameId & frame) 
	{
		...
		while(!foundSpace){
			if(!bufDescTable[clockHand].valid){
				...
				File* myFile = bufDescTable[clockHand].file;
				myFile->writePage(bufPool[clockHand]);
				frame = clockHand;
				foundSpace = true;
				hashTable->remove(bufDescTable[clockHand].file, bufPool[clockHand].page_number());
			}
			else{
				hashTable->remove(bufDescTable[clockHand].file, bufPool[clockHand].page_number());
				frame = clockHand;
		...
		for(FrameId i = 0; i < numBufs; i++){
			...
			if(currFrame.file == file){
				if(currFrame.dirty == true){
					...
					//PageId pageNo = currFrame.pageNo; unused variable
	...
	void BufMgr::allocPage(File* file, PageId &pageNo, Page*& page) 
	{
		...
		//std::cerr << "successfully saw line:" << __LINE__ << "\n";
